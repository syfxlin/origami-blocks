...
import "./style.scss";
import "./editor.scss";
import imageEdit from "../../tmp/edit";
import imageSave from "../../tmp/save";
...
registerBlockType("origami/notebox", {
	...
	edit: function(props) {
		...
		const selectedEle = wp.element.createElement(wp.components.SelectControl, {
			label: "选择样式",
...
registerBlockType("origami/prism", {
	...
	edit: function(props) {
		const contentEle = wp.element.createElement(wp.components.TextareaControl, {
			label: "Origami 代码块",
			value: props.attributes.content,
			onChange: function(val) {
				props.setAttributes({ content: val });
		...
		return (
			<div>
				<pre className={className1}>
					<code className={className2}>{props.attributes.content}</code>
				</pre>
			</div>
		);
	}
...
registerBlockType("origami/image", {
	...
	// edit: function(props) {
	// 	const isThumEle = wp.element.createElement(wp.components.CheckboxControl, {
	// 		label: __("是否设置为特色图片"),
	// 		value: props.attributes.isThum,
	// 		onChange: function(val) {
	// 			props.setAttributes({ isThum: val });
	// 		}
	// 	});
	// 	const isShowEle = wp.element.createElement(wp.components.CheckboxControl, {
	// 		label: __("是否设置为显示"),
	// 		value: props.attributes.isShow,
	// 		onChange: function(val) {
