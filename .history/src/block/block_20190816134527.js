...
registerBlockType("origami/prism", {
    ...
    edit: function(props) {
        ...
        let item = {};
        if (window.prism[props.attributes.hash]) {
            item = window.prism[props.attributes.hash];
        } else {
            window.prism[props.attributes.hash] = item;
            item.setting = {
                minLines: 10,
                fontSize: 17,
        ...
        const langEle = wp.element.createElement(wp.components.SelectControl, {
            ...
            onChange: function(val) {
                ...
                window.prism[props.attributes.hash].ace.session.setMode(
                    "ace/mode/" + val
                );
            }
        });
        const showMenuEle = wp.element.createElement(wp.components.Button, {
            content: "Click",
            isDefault: true,
            onClick: function() {
                console.log("click");
