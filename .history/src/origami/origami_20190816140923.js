...
registerBlockType("origami/markdown", {
    ...
    edit: function(props) {
        ...
        let item = {};
        if (window.origami["markdown"]) {
            item = window.origami["markdown"];
        } else {
            window.origami[props.attributes.hash] = item;
            item.setting = {
                minLines: 10,
            ...
            let timer = setInterval(() => {
                if (document.getElementById("markdown-editor")) {
                    clearInterval(timer);
                    item.ace = window.ace.edit("markdown-editor", item.setting);
                    item.ace.setValue(props.attributes.content);
                    item.ace.gotoLine(1);
                    item.ace.session.on("change", () => {
                        props.setAttributes({
                            content: item.ace.getValue()
                        });
                    });
                }
            }, 50);
        }
        return <div className={props.className} id="markdown-editor" />;
    },
    save: function(props) {
        return <div />;
    }
