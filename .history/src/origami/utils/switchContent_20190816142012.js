        ...
        if (/\[graff]{(.*)}/g.test(text)) {
            args[0] = text.replace(/\[graff]{(.*)}/g, function($1, $2) {
                return (
                    ...
                    window.eSetting.xkSetting.graffUrl +
                    "graff-" +
                    $2 +
                    ".png" +
                    '">'
...
export function toMarkdown(htmlVal, styleSwitch = true) {
    ...
    turndownService.addRule("mermaid", {
        filter: function(node) {
            return (
                node.nodeName === "PRE" &&
                node.classList.contains("xkeditor-mermaid")
            );
        },
    ...
    turndownService.addRule("math", {
        filter: function(node) {
            return (
                node.nodeName === "PRE" &&
                node.classList.contains("xkeditor-tex")
            );
        },
        replacement: function(content, node) {
    ...
    turndownService.addRule("graff", {
        ...
        replacement: function(content, node) {
            return (
                ...
                node
                    .getAttribute("src")
                    .replace(window.eSetting.xkSetting.graffUrl + "graff-", "")
                    .substr(0, 6) +
                "}"
            );
        }
